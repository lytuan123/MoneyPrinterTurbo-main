...
with middle_panel:
    with st.container(border=True):
        ...
        material_sources = [
            (tr("Pexels Video"), "pexels_video"),
            (tr("Pixabay Video"), "pixabay_video"),
            (tr("Pexels Image"), "pexels_image"),
            (tr("Pixabay Image"), "pixabay_image"),
        ...
        selected_index = st.selectbox(
            tr("Material Source"),
            options=range(len(material_sources)),
            format_func=lambda x: material_sources[x][0],
            index=saved_video_source_index,
        )
        material_source = material_sources[selected_index][1]

        # Determine if we're using video or image based on the source
        if material_source.endswith("_video"):
            params.video_source = material_source.replace("_video", "")
            params.material_type = MaterialType.video
        elif material_source.endswith("_image"):
            params.video_source = material_source.replace("_image", "")
            params.material_type = MaterialType.image
        else:
            params.video_source = material_source
            params.material_type = MaterialType.video

        if params.material_type == MaterialType.image:
            st.write(tr("Image to Video Settings"))

            # Image type selection (all, photo, animation)
            image_types = [
                (tr("All Images"), "all"),
                (tr("Photos Only"), "photo"),
                (tr("Animations Only"), "animation"),
            ]

            selected_index = st.selectbox(
                tr("Image Type"),
                options=range(len(image_types)),
                format_func=lambda x: image_types[x][0],
            )
            params.image_type = image_types[selected_index][1]

            # Number of images to use
            params.image_count = st.slider(
                tr("Number of Images"),
                min_value=5,
                max_value=50,
                value=20,
                step=5,
                help=tr("More images will create a more dynamic video")
            )

            # Image display duration
            params.image_duration = st.slider(
                tr("Image Duration (seconds)"),
                min_value=1.0,
                max_value=10.0,
                value=3.0,
                step=0.5,
                help=tr("How long each image will be displayed in the video")
            )
        ...
        params.video_concat_mode = video_concat_modes[selected_index][1]
...
if start_button:
    ...
    if params.video_source not in ["pexels", "pixabay", "local"]:
        st.error(tr("Please Select a Valid Material Source"))
        scroll_to_bottom()
        st.stop()
